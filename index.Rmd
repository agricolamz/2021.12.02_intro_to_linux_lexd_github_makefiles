...
Название этих материалов может выглядеть немного слишком амбициозным, однако, надеюсь, все понимают, что все, что перечислено в заглавии, является отдельной большой сложной темой, знание которой вы можете (скорее всего даже вам придется) углубить самостоятельно.
...
Флаги различаются по длине. Однобуквенные флаги можно перечислить вместе после минуса. Однако бывают длинные флаги, и для того, чтобы отличить их от комбинации коротких флагов, ставят два минуса:
...
[`lexd`](https://github.com/apertium/lexd) --- это программа для описания морфологии языка, разработанная в [apertium](https://apertium.org) (для этой цели также используют программы [`lexc` и `lttoolbox`](https://wiki.apertium.org/wiki/Lttoolbox_and_lexc)).
...
Обычно после преобразования полученное может быть использовано [инструментами hfst](https://wiki.apertium.org/wiki/Hfst) для работы с трансдьюссерами:
...
* добавьте еще один корень *устул*/*устулу*- 'стул';
* добавьте показатель датива *-й* и *-лъу*;
* добавьте показатели AD.ESSIVE *-ха*, AD.LATIVE *-хо*,  AD.ELLATIVE *-ха-кку* 
...
`git` --- это программа, которая позволяет следить и сохранять изменения, которые вы внесли в ваши программы. Существует множество сервисов как [GitHub](https://github.com/) и [GitLab](https://gitlab.com), которые предоставляют облачное хранение данных изменений. В результате, если ваша программа где-то сломалась или что-то пошло не так, вы всегда можете вернуться в любое предыдущее состояние программы, которое вас устраивает. Кроме того, `git` позволяет нескольким грамотным пользователям работать над одним проектом. `git` не единственная система, бывают еще CVS и SVN.

## Github

Для дальнейшей работы придется зарегестрироваться на гитхабе (вы можете подать [на студенческий аккаунт](https://education.github.com/pack), который имеет некоторые преимущества).

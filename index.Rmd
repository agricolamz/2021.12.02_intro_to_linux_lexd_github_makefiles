...
Мы видели, что чтобы скомпилировать анализатор и генератор форм нужен лишь маленький `.lexd` файл, который занимает всего 382 байта (в то время как бинарники весят 1240 байтов и 1730 байтов). В нашем случае, чтобы их сгенерировать нужно было всего две команды, однако на практике получаются куда более многоступенчатые процессы с массой промежуточных файлов. Инструкции по их созданию обычно записывают в файл, который называется `Makefile` (обратите внимание, что первая буква большая).


# Введение в `git`

```{r clean_after_yourself, include=FALSE}
files <- list.files() %in% c("and.noun.lexd", "index.Rmd", "index.html")
